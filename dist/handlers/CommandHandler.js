  ...
  constructor(handler, directory) {
    ...
    for (const [file, fileName] of files) {
      (() =>
        __awaiter(this, void 0, void 0, function* () {
          const command = (yield Promise.resolve().then(() =>
            __importStar(require(file))
          )).default;
          const {
            name = fileName,
            description,
          ...
          if (testOnly) {
            for (const server of handler.testServers) {
              ...
              yield handler.slashCmds.create(
                name,
                description,
                options,
                server
    ...
    this._client.ws.on("INTERACTION_CREATE", (interaction) => {
      if (interaction.type == 3) {
        const button = new ButtonInteraction_1.ButtonInteraction(
          interaction,
      ...
      if (
        devOnly &&
        !((_a = handler.botOwners) === null || _a === void 0
          ? void 0
          : _a.includes(interaction.member.user.id))
      ) {
      ...
      if (
        ...
        return interaction.reply(
          handler.permissionMsg.replace(
            /{PERMISSION}/g,
            utils_1.missingPermissions(interaction.member, perms)
          )
        );
      }
      const now = Date.now();
      const timestamp = handler.cooldowns.get(command.name);
      const amount = ms_1.default(
        (cooldown === null || cooldown === void 0
          ? void 0
          : cooldown.toString()) || "1s"
      );
        ...
        if (now < expirationTime) {
          return interaction.reply(
            handler.cooldownMsg.replace(
              /{COOLDOWN}/g,
              utils_1.msToTime(expirationTime - now)
            )
          );
        }
      }
